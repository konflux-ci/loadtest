...
def upload_file(s3_resource, local_name, bucket, remote_name):
    logger.debug(f"Going to upload {local_name}")
    ...
    logger.info(f"Uploaded {size}B of {local_name} as {remote_name}")
    return size


def get_presigned_url(s3_resource, bucket, remote_name):
    logger.debug(f"Going to generate signed URL for {remote_name}")
    ...
    logger.info(f"For {remote_name} obtained signed url {download_url}")
    return download_url
...
def delete_files(
    ...
    if objects_to_delete:
        ...
        logger.debug("object/objects deleted")
        return response
...
def download_file(s3_resource, bucket_name: str, remote_name: str, local_name: str):
    ...
    """
    logger.debug(f"Going to download {remote_name}")
    s3_bucket = s3_resource.Bucket(name=bucket_name)
    s3_object = s3_bucket.Object(key=remote_name)
    s3_object.download_file(Filename=local_name)
    logger.info(f"Downloaded {remote_name} to {local_name}")
