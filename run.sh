...
# Collect scheduler logs in background
if [ "${COLLECT_SCHEDULER_LOGS:-false}" = "true" ]; then
...
# Run the load test with profiling
go run loadtest.go \
...
if [ "${PROFILE:-false}" = "true" ]; then
    mv cpu.pprof "$output_dir/"
    mv mem.pprof "$output_dir/"
fi
...
# Collect pprof data if any
find "$output_dir" -name "*.pprof" -exec mv {} "$output_dir/" \;
